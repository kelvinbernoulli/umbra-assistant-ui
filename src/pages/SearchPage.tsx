    ...
    <div className="page page--search">
      <div className="page-intro search-intro">
        ...
        <p className="page-intro__copy">
          Search the messages, events, and documents you’ve saved.
        </p>
      </div>
      <SearchForm key={query} query={query} onSearch={choose} />
      {query ? (
        <ApiAccess>
          <SearchMatches key={query} query={query} />
        </ApiAccess>
      ) : (
        <div className="search-start">
          <div className="search-start__orb">
            <Search size={21} />
          </div>
          <h2>Search across your digital life</h2>
          <p>Find matching words across your connected sources.</p>
          ...
          <div className="privacy-note">
            <ShieldCheck size={16} />
            <span>
              <strong>Your memory stays yours.</strong> Sign in to search your
              workspace.
            </span>
          </div>
        </div>
  ...
  return (
    <section className="search-results">
      <div className="search-results__summary">
        ...
        <span className="page-intro__copy">Newest first</span>
      </div>
      <ApiState
        loading={isLoading || (!results && !error)}
        error={error}
        empty={
          results && !results.length
            ? "No matching items. Try different words."
            : undefined
        }
        onRetry={load}
      />
      <div className="result-list">
        {results?.map((result) => (
          <article className="result-item" key={result.id}>
            <div className="source-icon source-icon--manual">
              <Search size={18} />
            </div>
            <div className="result-item__body">
              ...
              <p className="record-detail-text">{result.snippet}</p>
            </div>
          </article>
        ))}
      </div>
      {results && (
        <p className="page-intro__copy dashboard-footnote">
          Up to 100 matches.
        </p>
      )}
    </section>
