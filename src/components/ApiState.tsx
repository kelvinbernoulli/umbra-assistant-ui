...
export function ApiAccess({ children }: { children: ReactNode; workspace?: boolean }) {
  ...
  if (sessionLoading) return <div className="empty-state"><LoadingIndicator label="Connecting to your workspace…" /></div>
...
export function ApiState({ loading, error, empty, onRetry }: {
  ...
  if (loading) return <div className="empty-state"><LoadingIndicator label="Loading from Umbra…" /></div>
